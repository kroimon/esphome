...
def to_code(config):
    ...
    if CONF_ACTIVE_STATE_DATAPOINT in config:
        ...
        if CONF_ACTIVE_STATE_HEATING_VALUE in config:
            cg.add(var.set_active_state_heating_value(config[CONF_ACTIVE_STATE_HEATING_VALUE]))
        if CONF_ACTIVE_STATE_COOLING_VALUE in config:
            cg.add(var.set_active_state_cooling_value(config[CONF_ACTIVE_STATE_COOLING_VALUE]))
    else:
        if CONF_HEATING_STATE_PIN in config:
            heating_state_pin = yield cg.gpio_pin_expression(config[CONF_HEATING_STATE_PIN])
            cg.add(var.set_heating_state_pin(heating_state_pin))
        if CONF_COOLING_STATE_PIN in config:
            cooling_state_pin = yield cg.gpio_pin_expression(config[CONF_COOLING_STATE_PIN])
